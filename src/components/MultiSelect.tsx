...
import { IconPlaceholder } from "./resgistry-helpers/icon-placeholder"
...
export function MultiSelect({
  ...
  const onItemAdded = useCallback((value: string, label: ReactNode) => { 
    //o que faz o useCallback? O que é um ReactNode ?
    setItems(prev => {
...
export function MultiSelectTrigger({
  ...
  const { open } = useMultiSelectContext() //Can you do that? Why not useContext(MultiSelectContext)? Cool! oh no, its just a function defined below
  return (
    //what is this asChild?
...
export function MultiSelectValue({
  ...
  const shouldWrap = 
    ...
    const handleResize = useCallback(
      (node: HTMLDivElement) => {
        ...
        const mutationObsever = new MutationObserver(checkOverflow)
        const observer = new ResizeObserver(debounce(checkOverflow, 100))

        mutationObsever.observe(node, {
          childList: true,
          attributes: true,
  ...
  return (
    <div
      ...
      ref={handleResize}
      className={
        cn("flex w-full gap-1.5 overflow-hidden",
